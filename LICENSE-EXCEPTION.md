...
is the `photos.sluice.application.port.out.VisionSieve` interface, together
with every Sluice type reachable from that interface's own method signatures
and their documented contract.

A type is reachable if a method of `VisionSieve` takes it as a parameter,
...
naming and populating the decision shards `sift` is defined to produce. A
...
1. It implements `VisionSieve` and registers itself as a Spring bean so Sluice
   can load it.
2. It uses no part of Sluice outside the SPI. It does not reproduce any of
   Sluice's own `VisionSieve` implementations either (for example
   `AnthropicSieve` or `ExternalAgentSieve`).
...
This permission does not extend to a module that implements `VisionSieve`
