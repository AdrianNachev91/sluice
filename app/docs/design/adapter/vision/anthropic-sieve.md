...
How `adapter/vision/AnthropicSieve` implements the `VisionSieve` port
(`app/src/main/java/photos/sluice/adapter/vision/AnthropicSieve.java`). It turns a prepared
...
```mermaid
flowchart TD
    A["sift(prep, opts)"] --> B["require sluice.sift.provider-settings.<br/>anthropic.model"]
    B --> C["render the system prompt<br/>(SievePrompt + the cards<br/>index.json recorded)"]
    ...
    U --> J
    T -- no --> X(["throw SiftException with<br/>both attempts' problems"])
    G -- clean --> I["write decisions-NNN.json"]
    I --> E
    E -- all done --> K(["SiftReport<br/>(sifted, skipped, token totals)"])
```
...
montage whose shard is already on disk and valid, so an interrupted sift finishes only the
remainder. An unreadable or contract-breaking existing shard is re-sifted and overwritten.
...
flowchart TD
    ...
    C --> D["SievePrompt.userTurn:<br/>scope line, grid line, photo table"]
...
entry, and `SiftMontageRenderer` keeps the reviewable and unreviewable sets disjoint. A shard read
...
**Retries are off and the timeout is short.** A sift rides the retry count the user configured; a
...
| Response fails validation twice (attempt + corrective retry)                        | Re-run the sift               | Checked `SiftException` naming the montage and both attempts' problems          |
| Response cut off at the token ceiling, twice                                        | Re-run, or use a smaller grid | Checked `SiftException` saying the answer was cut off, not that it was bad JSON |
...
| A verdict names the wrong photo for its index, twice          | `SiftException`; no shard written for that montage                                                                |
| A tile has no verdict, or two, twice                          | `SiftException` listing the gap or the duplicate                                                                  |
| Action is not `keep`, near-dup, or a recorded category, twice | `SiftException` via `ShardValidator`                                                                              |
| Two montages reuse one near-dup group id, twice               | `SiftException` at the second montage; the first montage's shard stays on disk                                    |
| Response is not the schema's JSON, twice                      | `SiftException`                                                                                                   |
| Response stops at `max_tokens`, twice                         | `SiftException` naming the ceiling rather than the JSON                                                           |
...
| Re-run after a failure or interruption                        | Montages with valid shards resume (skipped, no API call); the rest are sifted                                     |
| Existing shard unreadable or contract-breaking                | Re-sifted; the fresh shard overwrites it                                                                          |
...
- **`SiftOptions` is ignored.** A montage resumes, sifts, or is skipped for want of a readable
...
- **Exhaustive verdicts are always on.** The designed off-switch for cheap all-keeper scopes
  (`sift.exhaustiveVerdicts`) is not implemented.
...
- `adapter/vision/SievePrompt` - renders the system prompt and each montage's user turn. The
...
- `adapter/vision/ExternalAgentSieve` - the sibling provider; same output contract, judgement
  arrives from outside the app instead of an API call.
- `domain/sift/ShardValidator` - the shard contract's single source of truth.
- `application/port/out/VisionSieve` - the port this class implements;
  `application/service/SiftDispatcher` routes to it by the configured provider id.
...
- [`sift-montage-renderer.md`](../imaging/sift-montage-renderer.md) - keeps a montage's reviewable
