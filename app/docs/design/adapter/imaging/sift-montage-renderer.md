# Sift montage renderer

How `adapter/imaging/SiftMontageRenderer` implements the `MontageRenderer` port
(`app/src/main/java/photos/sluice/adapter/imaging/SiftMontageRenderer.java`). It wires
`TileRenderer`, `MontageBuilder`, `SidecarWriter`, and `PrepIndexWriter` together. Given a
`SiftScope`, it finds the right `Sorted` photo files and renders each to a tile. It then drops
...
```mermaid
flowchart TD
    A["SiftScopeSelector.directoriesToScan(photosRoot, scope)"] --> B{"Year or OldestN?"}
    ...
    D --> E
    E --> F["build a SiftCandidate<br/>(path + mtime) per file"]
    F --> G["SiftScopeSelector.order(candidates, scope)"]
    G --> H{"Year or OldestN?"}
...
`SiftScopeSelector` never touches disk itself; it's pure `Path`/`Instant` logic, so it stays
...
- **The montage-write loop** - checked once per montage, before writing it. Whatever montages the
  ...
  one would refuse every later sift of that scope. It holds nothing worth refusing over. No
...
cancellation later in the sift flow (dispatch or apply), which lands on an already-prepped,
...
- **Unreviewable files are reported, never moved.** `PrepDir.unreviewable` lists them so
  ...
  belongs to the apply step, which acts on sifted scopes. Keeping this class
  side-effect-free is also what keeps a sift run dry-run-safe.
...
- `adapter/imaging/SidecarWriter` / `PrepIndexWriter` - JSON emission for each montage and for the
  overall `index.json`.
- `domain/sift/SiftScopeSelector` + `SiftCandidate` - pure scope-resolution and ordering logic.
- `domain/sift/PrepDir` - the receipt this class returns, mirroring `index.json`'s field shape but
