...
One `move-records.log` lives inside each prep directory, at the path `logFor(prepDirPath)`
...
`read()` parses the whole file in one pass into four accumulators: `moves`, `skipped`,
`overlaps`, and `corruptSidecars`. A witnessed move and a reconstructed move share the same
`moves` map. Provenance is not kept as a separate field once parsed, so both are trusted
identically by a reading caller.
