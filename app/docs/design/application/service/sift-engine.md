...
How `application/service/SiftEngine` orchestrates a sift job: prep (`MontageRenderer.build`) ->
dispatch (`SiftDispatcher.sift`, which routes to whichever `VisionSieve` the configured provider
...
(`app/src/main/java/photos/sluice/application/service/SiftEngine.java`,
`app/src/main/java/photos/sluice/application/service/ShardTallyCalculator.java`). `Pipeline` builds
the one `SiftEngine` instance it needs and exposes `sift()`/`resume()` under its own type - see
...
by `SiftWatchers`
(`app/src/main/java/photos/sluice/application/service/SiftWatchers.java`,
`app/src/main/java/photos/sluice/application/service/SiftWatcher.java`). `SiftEngine` holds the one
`SiftWatchers` instance it needs and delegates to it - see the "Watching a waiting run" section
below.
...
**The unknown row is not `DAMAGED`, though it looks the same to a user.** `SiftEngine.occupancyOf`
...
The question is asked twice. `sift()` asks it synchronously before `JobRunner.submit()`, for the
...
decides whether a sieve is entered at all. The second is what a `SiftException` from dispatch
means, which depends on the provider. See `ProviderType`'s own doc comment for the full reasoning
on that one.

```mermaid
flowchart TD
    A["SiftEngine.sift(scope)"] --> K{"configured provider<br/>holds the key it needs?"}
    ...
    P -- "null" --> PZ(["SiftJobOutcome.Cancelled"])
    P -- "PrepDir" --> S{"any montage<br/>still owing a shard?"}
    S -- "no" --> AP["apply: ApplyEngine.apply<br/>-> ApplyReport (or null -<br/>see Cancellation)"]
    S -- "yes" --> D["dispatch: SiftDispatcher.sift<br/>(allowPartial=false on a fresh sift,<br/>caller-supplied on resume)"]
    D -- "stopped at its spend ceiling" --> CE(["SiftJobOutcome.Waiting<br/>- reason CEILING_REACHED,<br/>apply is not reached"])
    D -- "success" --> AP
    AP -- "ApplyReport" --> APP(["SiftJobOutcome.Applied"])
    AP -- "null" --> APZ(["SiftJobOutcome.Waiting"])
    AP -- "ApplyException" --> APB(["SiftJobOutcome.Blocked<br/>- carries the findings,<br/>watch stays disarmed"])
    AP -- "RuntimeException" --> APR(["propagates -<br/>recorded as FAILED first"])
    D -- "SiftException" --> M{"configured provider's<br/>type == MANUAL?"}
    M -- "yes" --> WT(["SiftJobOutcome.Waiting<br/>- slot released, not a failure"])
...
still sifting, or an automated run stopped part way. A watcher can usefully poll for that, so the
...
Once every montage has a shard, the sifting agent has said everything it is going to say. Re-asking
...
catch everything a sieve's own batch check would have. It does, across four families:
...
there is nothing for a sieve to judge, so dispatching would only produce an empty report.

`resume(prepDir, allowPartial)` re-reads the existing `PrepDir` from `index.json` via
`siftPrepPort.readIndex()` instead of `MontageRenderer` regenerating it, so no montage is ever
...
sift-prep root and diagnoses each dir, and `Pipeline.siftRuns()` exposes it - see
[`prep-dir-doctor.md`](prep-dir-doctor.md). `SiftEngine` reads it for two things only: the startup
...
`tally()` is the `present`/`valid`/`total` display number on each `WaitingSiftJob`, computed one
...
`isReadyToResume()` is what a `SiftWatcher` polls, and it asks a much narrower question: has
...
| SiftEngine method               | What runs                                                           | Phase label(s)                                                         |
|---------------------------------|---------------------------------------------------------------------|------------------------------------------------------------------------|
| `sift(SiftScope)`               | prep -> (dispatch unless the scope is empty) -> apply               | `"Building montages..."`, `"Sifting..."`, `"Applying decisions..."`    |
...
A fresh `sift()` over a scope with any montages in it always dispatches. Prep rebuilds the dir and
clears whatever was in it, so no montage can already hold a shard. The one fresh-sift case that
skips is a scope that produced no montages at all, where there is nothing for a sieve to judge.
...
| Scenario                                                            | Outcome                                                                                                                                                    |
|---------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sift()` on an external-agent scope with no shards dropped yet      | `SiftJobOutcome.Waiting` with a `present=0/valid=0` tally; job slot released                                                                               |
| `sift()` called again while that scope's run is unresolved          | `ScopeOccupiedException` thrown synchronously, before `JobRunner.submit()` - the existing prep dir (and any already-dropped shards) is left untouched      |
| `sift()` on a scope whose own prep dir cannot be listed at all      | `ScopeUnreadableException` thrown synchronously, naming the prep dir and the read failure - nothing rebuilt, no run fabricated to blame                    |
| `sift()` called again once that scope's run has applied             | The old record is archived to the graveyard, named by `archivedPriorRun`, and the fresh run proceeds with no confirm                                       |
| `resume()` once every shard is present and valid                    | `SiftJobOutcome.Applied`, files moved. No sieve is entered, so no `"Sifting..."` phase is bracketed                                                        |
| `resume()` while a shard is still missing                           | Dispatch runs again; `SiftJobOutcome.Waiting`, with a freshly recomputed tally                                                                             |
| `resume()` with every shard present but apply's validation refusing | `SiftJobOutcome.Blocked` carrying the findings; nothing moved, `decisions.json` never written, no watcher armed                                            |
...
| A genuine `SiftException` from an automated (non-MANUAL) provider   | Propagates - `JobHandle.join()` throws, never resolves to `Waiting`. A cancellation is a separate path (see Cancellation below) and never reaches this one |
| The run stops at its own spend ceiling                              | `SiftJobOutcome.Waiting` with reason `CEILING_REACHED`. Apply is not reached, so the shards already written stay unapplied and Continue picks them up.     |
...
Cancel is effectively Pause for sift, not a failure, at every boundary except one. `sift()`/
...
themselves. Every long-running pass in the whole sift flow now checks the same signal, not just the
two stage boundaries between them:

```mermaid
flowchart TD
    A["buildFreshAndDispatch(scope)"] --> P["prep: MontageRenderer.build<br/>(checks cancellation internally -<br/>see sift-montage-renderer.md)"]
    P -- "null - cancelled mid-render,<br/>nothing resumable yet" --> WC(["SiftJobOutcome.Cancelled"])
    P -- "PrepDir" --> C1{"cancellation<br/>requested?"}
    C1 -- "yes" --> W1(["Waiting - 0/N tally,<br/>dispatch never runs"])
    C1 -- "no" --> D["dispatch: SiftDispatcher.sift<br/>(signal passed through)"]
...
An automated provider's own montage loop (`AnthropicSieve`) checks the signal in its loop
condition and once more before its corrective retry. A cancellation therefore lands within at most
one montage call, plus rarely one retry call - never as a thrown `SiftException`.

The external-agent provider's dispatch is a single fast completeness check with nothing to
interrupt mid-call. Its own pause `SiftException` still resolves to `Waiting` as
...
`SiftJobOutcome.Cancelled` is the one outcome with nothing to resume. The renderer stopped before
`index.json` was ever written, so there is no prep dir yet to derive a `WaitingSiftJob` from. A
plain re-run of `sift()` on the same scope starts fresh. Every other cancellation path in this
...
| Scenario                                                                       | Outcome                                                                                                                           |
|--------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|
| Cancellation requested mid-render, before `index.json` is written              | `SiftJobOutcome.Cancelled` - nothing resumable exists yet                                                                         |
| Cancellation requested mid-batch (the montage-write loop), before `index.json` | `SiftJobOutcome.Cancelled`, same as mid-render - see [`sift-montage-renderer.md`](../../adapter/imaging/sift-montage-renderer.md) |
...
| Cancellation requested racing the provider's pause `SiftException`             | `Waiting`, same as an uncancelled pause, but no watcher is armed                                                                  |
...
`SiftWatchers` owns the watch lifecycle: which prep dirs currently have a `SiftWatcher` polling
them, and the arm/disarm/auto-resume rules around that. `SiftEngine` holds the one instance it
...
`SiftEngine.dispatchAndApply()` arms a `SiftWatcher` for the prep dir when the provider's own pause
resolves into `Waiting`. That is on top of the plain `Waiting` behavior above.

Only the external-agent provider is watched, and `SiftWatchers.armWatch()` returns at once for any
other. Watching exists to notice when the user's own sifting agent, running outside this app, drops
...
is: still Waiting, still listed, and still blocking a fresh sift of its scope. Only the polling
stops.
...
will not come back. There is nothing left for a poller to notice, and `SiftEngine`'s `disarmWatch()`
...
```mermaid
flowchart TD
    A["dispatchAndApply() lands<br/>on SiftJobOutcome.Waiting"] --> B{"configured provider's<br/>type == MANUAL?"}
    B -- "no" --> Z(["stay Waiting - unchanged"])
    B -- "yes" --> C["SiftWatcher armed,<br/>polling every watchPollInterval<br/>(2s in production)"]
...
an internal cadence rather than a `SiftSettings` field.

`disarmWatch()` runs at the very start of every `dispatchAndApply()` call, regardless of who
triggered it (a fresh `sift()`, a manual `resume()` click, or a watcher's own auto-resume). The
...
`SiftWatchers.disarmAll()` retires every poller at once, reached through `Pipeline.stopAllWatching()`.
...
| The configured provider is an automated one                         | No watcher ever arms; behavior is identical to the `sift()`/`resume()` section above                  |
...
- [`curate-engine.md`](curate-engine.md): sorts a scope, then reuses this class's
  `buildFreshAndDispatch()` for the sift stage.
- `JobRunner`/`JobHandle`/`JobWork` (the single-slot async executor `SiftEngine` submits onto): no
...
- `SiftMontageRenderer`:
  [`sift-montage-renderer.md`](../../adapter/imaging/sift-montage-renderer.md) in the
...
- `VisionSieve` (the port `SiftDispatcher.sift` routes to): see
