 ...
 * contain. The shapes and their counts live in {@code takeout-sidecar-shapes.md}, which is the only
 * surviving record of that survey.
 *
 * <p>The unit tests beside this one pick their fixtures to isolate a single rule. These do the
 * opposite. They are whatever Google actually emitted, so a rule change that looks harmless
 * against a hand-picked case still has to face the real distribution.
 *
 * <p>The fixture is a Markdown table so it stays readable as a document. Only its table rows are
