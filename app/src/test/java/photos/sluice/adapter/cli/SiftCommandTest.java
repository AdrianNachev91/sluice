...
class SiftCommandTest {
    ...
    void aGappedMonthListIsAcceptedAsIs(@TempDir final Path root) {
        ...
        verify(this.pipeline).sift(new SiftScope.Year(2019, List.of(6, 8, 11)));
    }
    ...
    void aCountTakesTheOldestPhotosInsteadOfAYear(@TempDir final Path root) {
        ...
        verify(this.pipeline).sift(new SiftScope.OldestN(30));
    }
    ...
    void aCompletedSiftReportsWhatApplyMoved(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Applied(zeroReport(), new ApplyReport(10, Map.of("Junk", 3), 1, 0, 0,
                List.of()), null, null));
    ...
    void aRunWaitingOnAnAgentNamesNoOfferSinceNothingMovesUntilTheyAnswer(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Waiting(job("2019"), WaitingReason.SHARDS_OUTSTANDING, zeroReport(), null));
    ...
    void aCancelledSiftThatBecameAWaitingRunExitsWaitingRatherThanCancelled(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Waiting(job("2019"), WaitingReason.CANCELLED, zeroReport(), null));
    ...
    void aCeilingStopReassuresNothingMoreWasSpent(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Waiting(job("2019"), WaitingReason.CEILING_REACHED, zeroReport(), null));
    ...
    void aBlockedRunPointsAtTroubleshootRatherThanListingItsFindings(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Blocked(job("2019"),
    ...
    void aSiftTheProviderCouldNotFinishIsBlockedRatherThanRefusedOrFailed(@TempDir final Path root) {
        when(this.pipeline.sift(any())).thenAnswer(_ -> this.runner.submit(_ -> {
            throw new CompletionException(new SiftException("montage-003 came back with no decisions"));
        }));
        ...
        assertThat(result.err()).doesNotContain("SiftException");
    }

    @Test
    void aRunCancelledBeforeAnyMontageRenderedExitsCancelled(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Cancelled(zeroReport(), null));
    ...
    void whatWasActuallySpentIsNotedWhenAnythingWas(@TempDir final Path root) {
        this.answering(new SiftJobOutcome.Applied(
    ...
    void anArchivedPriorRunIsNamedInItsNewLocation(@TempDir final Path root) {
        ...
        this.answering(new SiftJobOutcome.Applied(zeroReport(), new ApplyReport(1, Map.of(), 0, 0, 0, List.of()),
                graveyard, null));
    ...
    void aRefusalRaisedStartingTheJobPrintsNoEstimate(@TempDir final Path root) {
        when(this.pipeline.configuredProviderSpends()).thenReturn(true);
        when(this.pipeline.sift(any())).thenThrow(new IllegalArgumentException("no key"));
    ...
    private void answering(final SiftJobOutcome outcome) {
        when(this.pipeline.sift(any())).thenAnswer(_ -> this.runner.submit(_ -> outcome));
    }

    private static SiftJobOutcome.Applied applied() {
        return new SiftJobOutcome.Applied(zeroReport(), new ApplyReport(0, Map.of(), 0, 0, 0, List.of()), null, null);
    }
    ...
    private CliHarness.Result run(final Path root, final String... args) {
        ...
        return CliHarness.run(CliHarness.parser(
                new SiftCommand(this.pipeline, jobs, this.progress, mock(PathsPort.class))), args);
    }
