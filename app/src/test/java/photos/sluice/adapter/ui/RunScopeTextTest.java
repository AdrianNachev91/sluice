...
import photos.sluice.adapter.ui.RunScopeText.Typed;
...
class RunScopeTextTest {
    ...
    void aBareYearMeansTheWholeYear() {
        assertThat(RunScopeText.parse("2019")).isEqualTo(new Typed.OfYear(2019, List.of()));
    }

    @Test
    void theMonthsAreSortedAndSaidOnlyOnce() {
        assertThat(RunScopeText.parse("2019 8,6,6-7"))
                .isEqualTo(new Typed.OfYear(2019, List.of(6, 7, 8)));
    }
    ...
    void anEmptyMonthIsRefusedWhicheverSideOfTheCommaItIsOn(final String typed) {
        assertThat(RunScopeText.parse(typed)).isInstanceOf(Typed.Refused.class);
    }
    ...
    void thisSurfaceReadsTheMonthTextTheTableSays(final ScopeMonthCases.Case expected) {
        final Typed read = RunScopeText.parse("2019 " + expected.typed());

        if (expected.refused()) {
            assertThat(read).isInstanceOf(Typed.Refused.class);
        } else {
            assertThat(read).isEqualTo(new Typed.OfYear(2019, expected.months()));
        }
    }

    @Test
    void anEmptyFieldIsNotARefusal() {
        assertThat(RunScopeText.parse("   ")).isInstanceOf(Typed.Blank.class);
    }

    @Test
    void theUndatedFolderIsNamedWhateverTheCase() {
        assertThat(RunScopeText.parse(RunScopeText.UNDATED.toUpperCase(Locale.ROOT)))
                .isInstanceOf(Typed.Undated.class);
