 ...
 */
final class SiftOutcomeReport {

    private SiftOutcomeReport() {
    }
     ...
     */
    static CommandOutcome of(final SiftJobOutcome outcome, final Path duplicates,
                             final Function<Path, String> instructions) {
        return switch (outcome) {
            case final SiftJobOutcome.Applied applied -> appliedOutcome(applied, duplicates);
            case final SiftJobOutcome.Waiting waiting -> waitingOutcome(waiting, duplicates, instructions);
            case final SiftJobOutcome.Blocked blocked -> blockedOutcome(blocked);
            case final SiftJobOutcome.Cancelled cancelled -> cancelledOutcome(cancelled);
        };
    }
     ...
     *
     * @param incomplete {@link SiftException} what the provider could not finish, and why
     * @return {@link CommandOutcome} the outcome
     */
    static CommandOutcome incompleteOutcome(final SiftException incomplete) {
        final SiftReport report = incomplete.report();
        ...
        return CommandOutcome.blocked(SiftPayloads.incomplete(incomplete.getMessage(), report), lines, notes);
    }

    /**
     * A completed run: what apply moved.
     *
     * @param applied {@link SiftJobOutcome.Applied} the completed run
     * @param duplicates {@link Path} the Duplicates root the near-duplicate copies went to
     * @return {@link CommandOutcome} the outcome
     */
    private static CommandOutcome appliedOutcome(final SiftJobOutcome.Applied applied, final Path duplicates) {
        final List<String> lines = new ArrayList<>();
        lines.add(ResultLines.countLine("Photos looked at", applied.applyReport().reviewed()));
        ResultLines.addWhenAny(lines, "Sheets judged", applied.siftReport().montagesSifted());
        ResultLines.addWhenAny(lines, "Calls to your provider", applied.siftReport().apiCalls());
        ...
        return CommandOutcome.done(SiftPayloads.outcome(applied), lines, notes(applied));
    }
     ...
     * @return {@link CommandOutcome} the outcome
     */
    private static CommandOutcome waitingOutcome(final SiftJobOutcome.Waiting waiting,
        ...
        if (waiting.reason() != WaitingReason.SHARDS_OUTSTANDING) {
            return CommandOutcome.waiting(SiftPayloads.outcome(waiting),
        ...
        final List<String> lines = List.of(line, "", handover == null
                ...
                : handover);
        return CommandOutcome.waiting(SiftPayloads.outcome(waiting, handover), lines, notes(waiting));
    }
     ...
     *
     * @param blocked {@link SiftJobOutcome.Blocked} the blocked run
     * @return {@link CommandOutcome} the outcome
     */
    private static CommandOutcome blockedOutcome(final SiftJobOutcome.Blocked blocked) {
        ...
        return CommandOutcome.blocked(SiftPayloads.outcome(blocked), List.of(line), notes(blocked));
    }

    /**
     * A run stopped before montage rendering finished, leaving nothing on disk.
     *
     * @param cancelled {@link SiftJobOutcome.Cancelled} the cancelled run
     * @return {@link CommandOutcome} the outcome
     */
    private static CommandOutcome cancelledOutcome(final SiftJobOutcome.Cancelled cancelled) {
        return CommandOutcome.done(SiftPayloads.outcome(cancelled),
     ...
     */
    private static List<String> notes(final SiftJobOutcome outcome) {
        final List<String> notes = new ArrayList<>();
        spendNote(outcome.siftReport()).ifPresent(notes::add);
     ...
     */
    private static Optional<String> spendNote(final SiftReport report) {
