...
/**
 * The {@link VisionSieve} provider for a user whose vision judgement comes from an agent outside
 * this app. That agent reads the montages and drops a {@code decisions-NNN.json} shard per montage
 * into the prep directory on its own schedule. Calling {@code sift()} is therefore a
 * single-attempt presence check, never a wait. It asks one question: which montages still have no
 * shard file at all. It throws {@link SiftException} naming every montage that has none. The throw
 ...
 * wait on. The returned report counts a montage holding a shard as sifted and a waived one as
 * skipped. It carries zero tokens: the judgement happened outside this app, so no model tokens
 * were spent here.
 */
@Component
class ExternalAgentSieve implements VisionSieve {
     ...
     * @throws SiftException if any montage has no shard and allowPartial is off
     */
    @Override
    ...
    public SiftReport sift(final PrepDir prep, final SiftOptions opts, final ProgressCallback progress) throws SiftException {
        ...
        if (!opts.allowPartial() && !missing.isEmpty()) {
            throw new SiftException("The sifting for " + prep.scope() + " is incomplete ("
                    + missing.size() + " montage(s) still without a shard):\n - "
                    + String.join("\n - ", missing));
        }
        return new SiftReport(total - missing.size(), missing.size(), 0, TokenSpend.none(PROVIDER_ID), false);
    }
}
