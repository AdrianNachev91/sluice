...
import photos.sluice.domain.sift.SiftJudgement;
...
/**
 * Assembles the text of an automated sifting request. The system prompt is the bundled fixed
 * template with two things rendered into it. One is the run's category cards, each contributing
 * only its plain-words name and description. The other is {@link SiftJudgement}, which the
 ...
 * sifting, the same recorded set the response is then validated against. Rendering from live config
 * would let the prompt describe one rule set while validation judged another.
 *
 * <p>Grid dimensions come from the sift settings because the sidecar records no grid info. Prompt
 * assembly therefore assumes a sift runs with the same montage config that rendered its prep
 * directory.
 */
@Component
class SievePrompt {
    ...
    private static final String TEMPLATE_RESOURCE = "/sift/sieve-prompt.md";
     ...
     *
     * @param settings {@link SiftSettings} the sift settings, supplying the montage grid
     */
    SievePrompt(final SiftSettings settings) {
     ...
     * with no set-aside classes at all, silently gutting the sift's rule set - fail loud instead.
     ...
     *
     * @param categories a {@link List} of {@link SiftCategory}, the cards the run recorded at prep time
     * @return {@link String} the rendered system prompt
     */
    String systemPrompt(final List<SiftCategory> categories) {
     ...
     * @param categories a {@link List} of {@link SiftCategory}, the run's sift categories to render into the
     * template
     * @return {@link String} the template with both placeholders replaced
     */
    static String render(final String template, final List<SiftCategory> categories) {
        ...
        return template.replace(CATEGORIES_PLACEHOLDER, cards)
                .replace(JUDGEMENT_PLACEHOLDER, SiftJudgement.TEXT)
     ...
     *
     * @param card {@link SiftCategory} the category card to render
     * @return {@link String} the card's markdown section
     */
    private static String card(final SiftCategory card) {
     ...
     * Failing in the constructor surfaces that at startup rather than mid-sift.
     *
     * @return {@link String} the bundled prompt template's text
     */
    private static String loadTemplate() {
        try (final var input = SievePrompt.class.getResourceAsStream(TEMPLATE_RESOURCE)) {
            if (input == null) {
