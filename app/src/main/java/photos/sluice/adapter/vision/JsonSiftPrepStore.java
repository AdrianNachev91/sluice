...
/**
 * The {@link SiftPrepPort} implementation. It sits in the same package as {@link ShardCodec} and
 ...
 * apply-side engines' own tests live outside this package and wire a real {@link SiftPrepPort}
 * rather than a fake.
 */
@Component
public class JsonSiftPrepStore implements SiftPrepPort {
     ...
     */
    @Autowired
    JsonSiftPrepStore(final ShardCodec shardCodec, final SidecarReader sidecarReader) {
     ...
     */
    JsonSiftPrepStore(final ShardCodec shardCodec, final SidecarReader sidecarReader, final JsonMapper mapper) {
     ...
     * damaged index, instead of crashing {@link SiftCategory}'s own constructor.
     *
    ...
    /**
     * Writes index.json wholesale - the recovery-time counterpart to {@code SiftMontageRenderer}'s
    ...
    /**
     * The category set the prep dir was sifted under. Absent is malformed content, unlike an absent
     ...
     * config. Silently judging a run against rules it was never sifted under is exactly what
     ...
     * and quietly costs sift recall. Its length is held to the same ceiling, and so are the
     * examples beside it. Every ceiling here is {@link SiftCategory}'s own, read off it rather than
     * repeated, so no file this app wrote can be refused by a number that drifted from the one that
     * wrote it.
     *
     * <p>Every field is checked before any card is built, so {@link SiftCategory}'s own
     ...
     * @return a {@link List} of {@link SiftCategory} the recorded category cards
     */
    private static List<SiftCategory> requiredCategories(final @Nullable List<@Nullable RawCategory> values,
                                                         final Path indexPath) {
        if (values == null) {
            throw new MalformedPrepJsonException("Prep index " + indexPath + " has no categories",
                    new IOException("null categories"));
        }
        final var cards = new ArrayList<SiftCategory>();
     ...
     * @param raw {@link RawCategory} one recorded card
     * @param indexPath {@link Path} index.json's own path, used only for the error message
     * @return {@link SiftCategory} the card
     */
    private static SiftCategory cardUnder(final String name, final RawCategory raw, final Path indexPath) {
        final String description = raw.description();
        if (description == null || description.isBlank()) {
            throw new MalformedPrepJsonException("Prep index " + indexPath + " has a category '" + name
                    + "' with no description", new IOException("blank category description"));
        }
        if (description.length() > SiftCategory.maxDescriptionLength()) {
            throw new MalformedPrepJsonException("Prep index " + indexPath + " has a category '" + name
                    + "' whose description is longer than " + SiftCategory.maxDescriptionLength()
                    + " characters", new IOException("over-long category description"));
        }
        return new SiftCategory(name, description, boundedExamples(name, raw, indexPath), Boolean.TRUE);
    }
    ...
    private static List<String> boundedExamples(final String name, final RawCategory raw, final Path indexPath) {
        ...
        if (examples.size() > SiftCategory.maxExamples()) {
            throw new MalformedPrepJsonException("Prep index " + indexPath + " has a category '" + name
                    + "' offering more than " + SiftCategory.maxExamples() + " examples",
                    new IOException("too many category examples"));
        }
        for (final String example : examples) {
            if (example.length() > SiftCategory.maxExample()) {
                throw new MalformedPrepJsonException("Prep index " + indexPath + " has a category '"
                        + name + "' with an example longer than " + SiftCategory.maxExample()
    ...
    /**
     * One card's examples, as a list {@link SiftCategory} can take. Every recorded card is enabled
    ...
    public void writeMergedDecisions(final Path prepDir, final String scope, final List<Decision> decisions,
        ...
        final var document = new MergedDecisions(
                scope,
                decisions.stream().map(JsonSiftPrepStore::toRaw).toList(),
