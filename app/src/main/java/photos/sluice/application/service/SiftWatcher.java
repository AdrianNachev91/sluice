...
/**
 * Polls one waiting sift's shard status on a fixed interval until it is ready to resume, then
 ...
 * thread of its own.
 */
final class SiftWatcher {

    private static final Logger log = LoggerFactory.getLogger(SiftWatcher.class);
    private static final ThreadFactory DAEMON_THREADS = runnable -> {
        final var thread = new Thread(runnable, "sift-watcher");
    ...
    /**
     * Creates a watcher for one waiting sift, not yet running.
     *
     ...
     */
    SiftWatcher(final Duration pollInterval, final BooleanSupplier isReady, final BooleanSupplier tryConsume) {
    ...
    private void poll() {
        try {
            ...
            log.warn("Sift watcher poll failed, will retry next tick", e);
        }
    }
