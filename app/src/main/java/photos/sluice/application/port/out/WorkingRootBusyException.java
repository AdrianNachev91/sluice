...
/**
 * Thrown when a working root cannot be claimed because another process already holds it. It carries
 * the root, so a caller names what it could not open without parsing the message.
 *
...
public final class WorkingRootBusyException extends IllegalStateException {
    ...
    public WorkingRootBusyException(final Path workingRoot, final @Nullable Throwable cause) {
        super("Another Sluice process is already using " + workingRoot
                + " - close it, then try again.", cause);
