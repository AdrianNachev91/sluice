 ...
 * way the work stays inside {@link #sift}, so callers invoke it uniformly regardless of provider.
 * What a caller still has to tell apart is what a {@link SiftException} out of {@link #sift} means,
 * and {@link #type} is what answers that.
 */
public interface VisionSieve {
    ...
    /**
     * How this provider gets its judgements, which tells a caller what a {@link SiftException} out
     * of {@link #sift} means.
     *
     ...
     *
     * <p>Settings tells the user this costs nothing, distinct from a sift. A provider with no free
     ...
     * @return {@link ProviderCheck} what the provider said
     */
    default ProviderCheck check(final SiftProviderSettings candidate) {
        return this.check();
    }
     ...
     * SiftException when it cannot. That throw is the signal to whoever is sifting to try again.
     ...
     * @param opts {@link SiftOptions} options controlling how the sieve runs
     * @return {@link SiftReport} a report of what the run did and spent
     */
    SiftReport sift(PrepDir prep, SiftOptions opts) throws SiftException;

    /**
     * Progress-aware sibling of sift() above. Defaulted to silently ignore progress so an
     ...
     * @return {@link SiftReport} a report of what the run did and spent
     */
    default SiftReport sift(final PrepDir prep, final SiftOptions opts, final ProgressCallback progress) throws SiftException {
     ...
     * @return {@link SiftReport} a report of what the run did and spent
     */
    default SiftReport sift(final PrepDir prep, final SiftOptions opts, final ProgressCallback progress,
                            final CancellationSignal cancellation)
            throws SiftException {
        return this.sift(prep, opts, progress);
    }
}
