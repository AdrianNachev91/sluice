 ...
 * sift over whatever that sort just populated.
 *
 * <p>It is a convenience for a caller that wants to offer curate as one action instead of two
 * separate calls to {@link SortUseCase} and {@link SiftUseCase}. It never commits Sorted to the
...
public interface CurateUseCase {
    ...
    /**
     * Sorts scope into Sorted, then sifts whatever that sort just populated.
     *
     * @param scope {@link SortScope} which Inbox files to sort
     * @return {@link CurateOutcome} the sort summary plus the resulting sift outcome, if any
     */
