...
/**
 * Thrown by {@link VisionSieve#sift} when the provider cannot yield a complete, valid set of
 ...
 *
 * <p>That lets the sieve fix everything in one pass instead of one error per re-run. Checked,
 * because an incomplete sift is an expected, recoverable outcome: the caller re-runs once the
 ...
 * replaces.
 */
public class SiftException extends Exception {

    private final transient @Nullable SiftReport report;
     ...
     */
    public SiftException(final String message) {
        this(message, (SiftReport) null);
    }
     ...
     *
     * @param message {@link String} the aggregated validation problems
     * @param report {@link SiftReport} what the run had judged and consumed, or null when the
     *        provider consumes nothing and counts nothing
     */
    public SiftException(final String message, final @Nullable SiftReport report) {
        super(message);
        this.report = report;
    }
     ...
     */
    public SiftException(final String message, final Throwable cause) {
        super(message, cause);
        this.report = null;
    }
     ...
     *
     * @return {@link SiftReport} the run's own report up to the throw, or null
     */
    public @Nullable SiftReport report() {
        return this.report;
    }
}
