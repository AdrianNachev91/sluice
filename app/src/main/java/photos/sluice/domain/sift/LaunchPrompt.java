 ...
 *
 * <p>Sluice ships no vision provider. On this route the reader supplies the intelligence, and the
 * app supplies only the folder and the contract. So this text is the whole of what stands between
 * a folder of sheets and an agent that can answer it.
 *
...
public final class LaunchPrompt {
    ...
    private static String body(final PrepDir prep) {
        return """
                ...
                ShardValidator.GROUP_SLUG_MAX_LENGTH, SiftJudgement.TEXT, categories(prep.categories()));
    }
     ...
     *
     * @param categories a {@link List} of {@link SiftCategory} the set the run was prepped under
     * @return {@link String} the names, comma-separated
     */
    private static String categoryNames(final List<SiftCategory> categories) {
        return categories.stream().map(SiftCategory::name).collect(Collectors.joining(", "));
    }
     ...
     */
    private static String categories(final List<SiftCategory> categories) {
     ...
     *
     * @param category {@link SiftCategory} the card
     * @return {@link String} the block
     */
    private static String category(final SiftCategory category) {
