...
/**
 * One user-configured classification category. The {@code name} field is the action id a sifting
 ...
 * into its sifting prompt. {@code examples} are the sample subjects rendered beside it.
 *
 * <p>Name and description are required. A blank name is unroutable. A blank description would
 * render a hollow prompt section and silently degrade sift recall, so construction fails loud
 * instead. Examples are optional, and an empty list is how a card says it offers none.
 *
 * <p>{@code enabled} says whether a sift routes to this card at all. A disabled card is left out of
 ...
 * switched off is on.
 *
 * <p>The name also becomes the folder a sifted file is moved into, which is what constrains its
 ...
 */
public record SiftCategory(String name, String description, List<String> examples, Boolean enabled) {
     ...
     * @param enabled {@link Boolean} whether a sift routes to this card, null meaning it does
     */
    public SiftCategory {
     ...
     */
    public static SiftCategory of(final String name, final String description) {
        return new SiftCategory(name, description, List.of(), Boolean.TRUE);
    }
